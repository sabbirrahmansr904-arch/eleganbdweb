...
// Replace top firebase imports with Supabase initialization
...
// Remove firebase imports and init
...
// Replace OTP database operations in server.ts
...
// Any remaining steadfast config references or banner hero / branding
code = code.replace(/const sfSnap = await getDoc\(doc\(db,\s*'config',\s*'steadfast'\)\);/g, `const sfSnap = await sbGetDoc('config', 'steadfast');`);
...
console.log('server.ts successfully patched to use Supabase instead of Firestore!');
