...
const generateSeedReviews = (): ReviewItem[] => {
  ...
  const banglaComments = [
    'মানসম্মত পণ্য—যারা নিতে চান তারা নিশ্চিন্তে নিতে পারেন। আমি তাদের ডিসপ্লে সেন্টার থেকে শার্টগুলো নিয়েছি। খুবই সুন্দর এবং আরামদায়ক। ধন্যবাদ, Man\'s Avenue!',
    ...
    'ডেলিভারি ভাইয়ের ব্যবহার খুব ভালো ছিল এবং প্রোডাক্ট সময়মতো পেয়েছি। Man\'s Avenue এর কাস্টমার সার্ভিস সত্যিই প্রশংসনীয়।',
    ...
    'আমি নিয়মিত ফরমাল কাপড় কিনি, কিন্তু Man\'s Avenue এর মেটেরিয়াল এবং প্রাইস অন্য সবার থেকে সেরা লেগেছে।',
    'ফেব্রিকটা অনেক প্রিমিয়াম ফিল দেয়। গরম বা শীত উভয় ঋতুতেই পরার জন্য পারফেক্ট।',
    'খুবই ক্লাসি ডিজাইন। বন্ধুদের অনেকেই জিজ্ঞেস করেছে কোথা থেকে কিনেছি। Man\'s Avenue এর জন্য শুভকামনা!',
...
export default function Reviews() {
  ...
  const [localCustomReviews, setLocalCustomReviews] = useState<ReviewItem[]>([]);
  ...
  useEffect(() => {
    try {
      ...
      if (cached) {
        setLocalCustomReviews(JSON.parse(cached));
      }
    } catch (e) {
  ...
  // Combine local custom reviews, Firestore reviews, and seed reviews
  const combinedReviewsMap = new Map<string, ReviewItem>();
  // Add seed reviews first
  seedReviews.forEach(r => combinedReviewsMap.set(r.id, r));
  // Override / add Firestore reviews
  firestoreReviews.forEach(r => combinedReviewsMap.set(r.id, r));
  // Override / add local custom reviews so they appear at top
  localCustomReviews.forEach(r => combinedReviewsMap.set(r.id, r));

  const allReviews = Array.from(combinedReviewsMap.values()).sort((a, b) => b.createdAt - a.createdAt);
            ...
            {filteredReviews.map((rev) => (
              ...
              >
                ...
                <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between text-[11px] text-gray-400">
                  ...
                  <span className="font-mono text-[10px] text-gray-300">Man's Avenue Verified</span>
                </div>
              </motion.div>
            ))}
          </div>
