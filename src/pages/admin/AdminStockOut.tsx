...
export default function AdminStockOut() {
  ...
  // Verify and Prepare rows for stock dispatching
  const handleVerifyAndPrepare = () => {
    ...
    filledRows.forEach(row => {
      ...
      const deductions = Object.entries(row.quantities)
        ...
        .map(([size, qty]) => {
          ...
          return {
            ...
            target: Math.max(0, currentStock - qty),
            isInsufficient: currentStock < qty
          };
        });
  ...
  // Final Commit of the stock out payload
  const handleCommitStockOut = async () => {
    const validItems = preparedItems.filter(item => item.isValid);
    if (validItems.length === 0) {
      toast.error('No valid products to stock out.');
      return;
    }
    ...
    for (const item of validItems) {
      ...
      item.deductions.forEach((ded: any) => {
        updatedSizeStock[ded.size] = ded.target;
        totalDeducted += ded.qty;
        if (ded.isInsufficient) {
      ...
      try {
        ...
        // Convert deductions array to flat quantities object for transaction
        const quantitiesObj: Record<string, number> = {};
        item.deductions.forEach((ded: any) => {
          quantitiesObj[ded.size] = ded.qty;
        });
        ...
        await addTransaction({
          ...
          notes: notes.trim() ? notes.trim() : 'Manual inventory dispatch'
        });
    ...
    if (successCount > 0) {
      toast.success(`Successfully dispatched ${successCount} products!`);
      if (warningCount > 0) {
        toast.error(`${warningCount} products had sizes with insufficient stock (deducted to 0)`);
      }
      // Reset rows
      setRows([
        { id: '1', sku: '', quantities: {} },
  ...
  // Memoized and sorted outflow historical transactions list
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-8 pb-20 font-sans bg-[#F8F9FD]">
      ...
      <div className="bg-[#E6ECF4] border border-white/90 rounded-3xl p-6 shadow-[-6px_-6px_16px_rgba(255,255,255,0.95),6px_6px_18px_rgba(165,180,205,0.35)] text-center flex flex-col items-center justify-center space-y-1.5">
        <h1 className="text-3xl font-black text-[#EF4444] tracking-tight flex items-center justify-center gap-2">
          Stock Out
        </h1>
        <p className="text-sm font-bold text-slate-500">
          Dispatch inventory and systematically deduct from available tracking stock.
        </p>
      </div>
          ...
          <div className="bg-[#F8F9FD] rounded-3xl p-6 border border-[#EFF2F6] shadow-[0_4px_24px_rgba(0,0,0,0.01)] space-y-5">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              ...
              <button 
                ...
                className="text-[10px] font-extrabold text-[#EF4444] hover:underline uppercase tracking-wider cursor-pointer"
              >
            ...
            {/* Scrollable grid container for sizes matching the screenshot */}
          ...
          <div className="bg-[#F8F9FD] rounded-3xl border border-[#EFF2F6] shadow-[0_4px_24px_rgba(0,0,0,0.01)] overflow-hidden flex flex-col">
                            ...
                            <td key={size} className="p-1 border-r border-gray-100">
                              <input 
                                ...
                                className={cn(
                                  ...
                                  hasVal 
                                    ? "text-[#EF4444] bg-red-50/60 border-red-500/20 shadow-inner" 
                                    : "text-red-500/60 bg-red-50/10"
                                )}
                              />
                            </td>
                          );
            ...
            <div className="p-6 border-t border-gray-100 bg-gray-50/60 flex flex-col md:flex-row justify-between items-center gap-6">
              ...
              <button 
                ...
                className="w-full md:w-auto px-6 py-3 bg-[#2563EB] hover:bg-[#1D4ED8] text-white rounded-xl text-xs font-bold transition-all shadow-sm active:scale-[0.98] flex items-center justify-center gap-2 cursor-pointer"
              >
            ...
            >
              ...
              <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                <div className="flex items-center space-x-2.5">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-gray-900">Verify Stock Out Batches</h3>
                    <p className="text-[10px] text-gray-500 font-medium">Verify catalog mapping, available stock, and stock reductions before committing.</p>
                  </div>
                </div>
                <button 
              ...
              <div className="p-6 overflow-y-auto space-y-6 flex-1">
                ...
                {preparedItems.some(item => item.isValid && item.deductions.some((d: any) => d.isInsufficient)) && (
                  <div className="bg-red-50 border border-red-100 p-4 rounded-2xl flex items-start gap-3">
                    <AlertCircle className="text-red-500 shrink-0 mt-0.5" size={16} />
                    <div className="text-xs text-red-800 space-y-1">
                      <p className="font-bold">Insufficient Stock Warning</p>
                      <p>Some of the requested quantities exceed current available stock. Quantities will be deducted to 0.</p>
                    </div>
                  </div>
                )}
                      ...
                      <div key={item.rowId} className={cn("p-4 flex flex-col md:flex-row md:items-center justify-between gap-4", !item.isValid && "bg-red-50/20")}>
                        ...
                        {item.isValid && (
                          <div className="flex flex-wrap gap-2 md:justify-end">
                            {item.deductions.map((ded: any) => (
                              <div key={ded.size} className={cn(
                                "flex items-center space-x-1.5 border px-2.5 py-1 rounded-lg text-xs",
                                ded.isInsufficient 
                                  ? "bg-amber-50 border-amber-200 text-amber-800 font-semibold" 
                                  : "bg-red-50 border-red-100 text-red-800"
                              )}>
                                <span className="font-bold text-[#EF4444]">{ded.size}:</span>
                                <span className="font-extrabold text-red-700">-{ded.qty}</span>
                                <span className="text-[10px] text-gray-400">({ded.current} → {ded.target})</span>
                              </div>
                            ))}
                          </div>
                        )}
              ...
              <div className="p-6 border-t border-gray-100 bg-gray-50 flex items-center justify-end space-x-3">
                ...
                <button 
                  onClick={handleCommitStockOut}
                  className="px-5 py-2.5 bg-[#EF4444] hover:bg-red-600 text-white rounded-xl text-xs font-bold transition-all shadow-sm flex items-center gap-1.5 cursor-pointer"
                >
                  <CheckCircle2 size={14} />
                  Confirm & Commit Stock Out
                </button>
              </div>
            </motion.div>
          </div>
      ...
      <div className="bg-[#F8F9FD] rounded-3xl border border-[#EFF2F6] shadow-[0_4px_24px_rgba(0,0,0,0.01)] overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-gray-900">Historical Outflow Logs</h3>
            <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider">Immutable archive of matrix stock-out records</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50">
                ...
                <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-wider text-gray-400 border-b border-gray-100 text-right">Outflow Total</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {recentTransactions.map((t) => (
                <tr key={t.id} className="hover:bg-gray-50/30 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    ...
                    <p className="text-[10px] text-[#EF4444] font-bold uppercase tracking-wider mt-0.5">{t.category}</p>
                  </td>
                  <td className="px-6 py-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {Object.entries(t.quantities).map(([size, qty]) => (qty as number) > 0 && (
                        <div key={size} className="flex items-center gap-1 bg-gray-50 border border-gray-200/50 px-2 py-0.5 rounded-md text-[10px] font-semibold text-gray-600">
                          <span className="text-[#EF4444] font-bold">{size}:</span>
                          <span>{qty}</span>
                        </div>
                      ))}
                    </div>
                  ...
                  <td className="px-6 py-4 text-right">
                    <span className="text-sm font-extrabold text-[#EF4444]">-{t.totalQuantity}</span>
                  </td>
                </tr>
              ))}
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-16 text-center">
                    <div className="inline-flex flex-col items-center gap-3 text-gray-300">
                      <PackageMinus size={36} strokeWidth={1.5} />
                      <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">No Historical Stock Out Logs</p>
                    </div>
                  </td>
                </tr>
              )}
