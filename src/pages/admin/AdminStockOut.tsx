...
export default function AdminStockOut() {
  ...
  const handleQuantityChange = (size: string, val: string) => {
    ...
    // Check if available stock for this size is sufficient
    const currentSizeStock = Number(
      selectedProduct?.sizeStock?.[size] ?? 
      0
    );

    if (num > currentSizeStock) {
      toast.error(`সতর্কতা: ${size} সাইজে বর্তমানে পর্যাপ্ত স্টক নেই (মজুদ আছে: ${currentSizeStock} পিস)।`);
    }
  ...
  const handleSubmitStockOut = async (e: React.FormEvent) => {
    ...
    if (totalQuantityOut <= 0) {
      toast.error('অন্তত ১ পিস পরিমাণ স্টক আউট করুন।');
      return;
    }

    // Validate against current stock
    const currentTotalStock = selectedProduct.sizes && selectedProduct.sizes.length > 0
      ? selectedProduct.sizes.reduce((s, sz) => s + (Math.max(0, Number(selectedProduct.sizeStock?.[sz]) || 0)), 0)
      : (selectedProduct.stock || 0);

    if (totalQuantityOut > currentTotalStock) {
      toast.error(`ত্রুটি: মোট মজুদের চেয়ে বেশি স্টক আউট করা সম্ভব নয় (মজুদ: ${currentTotalStock} পিস)।`);
      return;
    }
    ...
    try {
      ...
      await addTransaction({
        ...
        notes: notes.trim() || 'Stock Out entry'
      });

      // 2. Update product stock & sizeStock accurately (decrement)
      ...
      Object.entries(quantities).forEach(([sz, qty]) => {
        ...
        updatedSizeStock[sz] = Math.max(0, currentQty - qty);
      });

      const updatedSizes = Array.isArray(selectedProduct.sizes) && selectedProduct.sizes.length > 0
      ...
      toast.success(`সফলভাবে ${totalQuantityOut} পিস স্টক আউট করা হয়েছে!`);
      ...
      toast.error('স্টক আউট সংরক্ষণ করতে সমস্যা হয়েছে। আবার চেষ্টা করুন।');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const stockOutTransactions = useMemo(() => {
    return transactions.filter(t => t.type === 'out').slice(0, 25);
  }, [transactions]);
      ...
      <div className="bg-white border border-gray-200/80 rounded-3xl p-6 sm:p-8 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 font-medium">
                ড্যামেজ, রিটার্ন বা শোরুম ডেলিভারির জন্য ডেটাবেজ থেকে স্টক আউট রেকর্ড করুন
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 bg-[#F8F9FD] border border-gray-200/80 px-5 py-3 rounded-2xl">
          <div className="text-right">
            <p className="text-[10px] text-gray-400 font-black uppercase tracking-wider">মোট স্টক আউট রেকর্ড</p>
            <p className="text-lg font-black text-rose-600 leading-none">{stockOutTransactions.length}</p>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-6 bg-white border border-gray-200/80 rounded-3xl p-6 sm:p-8 shadow-xs space-y-6">
          <div className="flex items-center justify-between border-b border-gray-100 pb-4">
            <h4 className="text-sm font-black text-gray-900 uppercase tracking-widest flex items-center gap-2">
              <Minus size={16} className="text-rose-600" /> নতুন স্টক আউট এন্ট্রি ফরম
            </h4>
            <span className="text-[10px] font-bold uppercase px-2.5 py-1 bg-rose-50 text-rose-700 rounded-lg border border-rose-200">
              ডাটাবেজ কানেক্টেড
            </span>
          </div>

          <form onSubmit={handleSubmitStockOut} className="space-y-6">
                  ...
                  {filteredProductsForSelect.map(p => (
                    <div
                      ...
                      onClick={() => {
                        ...
                        const initialQty: Record<string, number> = {};
                        sizes.forEach(s => { initialQty[s] = 0; });
                        setQuantities(initialQty);
                      }}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] font-mono text-gray-400">SKU: {p.sku || p.id} • স্টক: {p.stock || 0} পিস</p>
                        </div>
                      </div>
              ...
              <div className="bg-[#F8F9FD] border border-gray-200 rounded-2xl p-5 space-y-4 shadow-inner">
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-black uppercase text-gray-700 tracking-wider">
                    সাইজ অনুযায়ী আউট করার পরিমাণ ({currentSizes.join(', ')}):
                  </label>
                  <div className="grid grid-cols-4 sm:grid-cols-7 gap-2">
                    {currentSizes.map(size => {
                      const avail = Number(selectedProduct.sizeStock?.[size] || (selectedProduct.sizes as any)?.[size] || 0);
                      return (
                        <div key={size} className="bg-white border border-gray-200 rounded-xl p-2 text-center shadow-xs">
                          <span className="text-[10px] font-black uppercase text-gray-500 block mb-0.5">{size}</span>
                          <span className="text-[9px] text-gray-400 block mb-1">মজুদ: {avail}</span>
                          <input 
                            type="number"
                            min="0"
                            max={avail}
                            ...
                            className="w-full bg-[#F8F9FD] border border-gray-200 rounded-lg py-1 text-center text-xs font-black text-rose-600 outline-none focus:border-black"
                          />
                        </div>
                      );
                    })}
                ...
                <div className="pt-2 flex items-center justify-between border-t border-gray-200">
                  <span className="text-xs font-black text-gray-700 uppercase">মোট আউট হচ্ছে:</span>
                  <span className="text-base font-black text-rose-600">{totalQuantityOut} পিস</span>
                </div>
              </div>
            ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  placeholder="যেমন: শোরুম ডেলিভারি / ড্যামেজ"
                  className="w-full bg-[#F8F9FD] border border-gray-200 rounded-xl px-4 py-3 text-xs font-bold text-gray-900 outline-none focus:border-black"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={submitting || !selectedProduct || totalQuantityOut <= 0}
              className={cn(
                ...
                submitting || !selectedProduct || totalQuantityOut <= 0
                  ? "bg-gray-300 cursor-not-allowed shadow-none"
                  : "bg-rose-600 hover:bg-rose-700 shadow-rose-200"
              )}
            >
              {submitting ? <RefreshCw size={16} className="animate-spin" /> : <CheckCircle2 size={16} />}
              {submitting ? 'প্রসেসিং হচ্ছে...' : `স্টক আউট কনফার্ম করুন (${totalQuantityOut} পিস)`}
            </button>
          </form>
        </div>

        {/* Right: Recent Stock Out Logs */}
        <div className="lg:col-span-6 bg-white border border-gray-200/80 rounded-3xl p-6 sm:p-8 shadow-xs space-y-6">
          <div className="flex items-center justify-between border-b border-gray-100 pb-4">
            <h4 className="text-sm font-black text-gray-900 uppercase tracking-widest flex items-center gap-2">
              <Calendar size={16} className="text-rose-600" /> সাম্প্রতিক স্টক আউট ইতিহাস (Stock Out History)
            </h4>
            <span className="text-xs font-bold text-gray-400 font-mono">
              মোট: {stockOutTransactions.length}
            </span>
          </div>
          ...
          <div className="space-y-3 max-h-[580px] overflow-y-auto pr-1">
            {stockOutTransactions.map((tx) => (
              <div 
                key={tx.id}
                className="bg-[#F8F9FD] border border-gray-200/80 rounded-2xl p-4 space-y-2.5 shadow-xs hover:border-rose-300 transition-all"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="px-2.5 py-0.5 bg-rose-600 text-white rounded-md text-[10px] font-mono font-bold uppercase">
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {Object.entries(tx.quantities || {}).filter(([_, q]) => Number(q) > 0).map(([sz, q]) => (
                      <span key={sz} className="px-2 py-0.5 bg-white border border-gray-200 rounded-lg text-[10px] font-bold text-gray-700">
                        {sz}: <span className="text-rose-600 font-black">-{q}</span>
                      </span>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-200/60 text-[10px] text-gray-500">
                  <span>কর্তৃক আউট: <strong className="text-gray-800">{tx.authorizedBy || 'Admin'}</strong></span>
                  {tx.notes && <span className="italic text-gray-400">কারণ: {tx.notes}</span>}
                </div>
              </div>
            ))}
            ...
            {stockOutTransactions.length === 0 && (
              <div className="py-20 text-center space-y-3 bg-[#F8F9FD] border border-gray-200 rounded-2xl">
                ...
                <p className="text-xs font-black uppercase text-gray-500">কোনো স্টক আউট রেকর্ড পাওয়া যায়নি</p>
              </div>
            )}
          </div>
        </div>
